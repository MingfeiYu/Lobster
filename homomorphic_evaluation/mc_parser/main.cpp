...
int main( int argc, char **argv )
{
    ...
    if ( argc == 3 )
    {
        ...
        m = helib::FindM( security, L, c, p, d, s, 0 );
        cout << "selected m : " << m << endl;
        helib::Context context( m, p, r );
        context.buildModChain( L, c );

        helib::SecKey sk( context );
        helib::PubKey const& pk = sk;
        ...
        for ( auto i = 0 ; i < eqnlist.size(); ++i)
        {
            ...
            else if ( top_op == MC::Bexp::Head::XOR )
            {
                ...
                /* for debugging */
                // ZZX xor_res;
                // sk.Decrypt( xor_res, lchild_ctxt );
                // cout << lchild_var << " + " << rchild_var << " = " << lv << endl;
                // cout << lchild_res[0] << " + " << rchild_res[0] << " = " << xor_res[0] << endl;
            ...
            else if ( top_op == MC::Bexp::Head::OR )
            {
                ...
                helib::Ctxt tmp_ctxt1 = lchild_ctxt;
                ...
                /* for debugging */
                // ZZX or_res;
                // sk.Decrypt( or_res, lchild_ctxt );
                // cout << lchild_var << " or " << rchild_var << " = " << lv << endl;
                // cout << lchild_res[0] << " or " << rchild_res[0] << " = " << or_res[0] << endl; 
            ...
            else if( top_op == MC::Bexp::Head::NOT )
            {
                ...
                /* for debugging */
                // ZZX not_res;
                // sk.Decrypt( not_res, rchild_ctxt );
                // cout << "not " << rchild_var << " = " << lv << endl;
                // cout << "not " << rchild_res[0] << " = " << not_res[0] << endl;
        ...
        if ( circuit_filename.find( opted_filename_tag ) != std::string::npos )
        {
            ...
            {
                cerr << string( 16 - time_length, ' ' ) << time_string;
                cerr << endl;
            }
            else
            {
                cerr << string( 19 - time_length, ' ' ) << time_string;
            }
        }
        else
        {
            cerr <<  circuit_filename << string( 15 - name_length, ' ' )  << string( 12 - time_length, ' ' ) << time_string ;
        }
    }
